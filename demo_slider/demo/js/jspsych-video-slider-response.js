...
jsPsych.plugins["video-slider-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    //<input type="range" class="jspsych-slider" value="'+trial.slider_start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" 
    html += '">';
    html += '<div style="width: 100%;" id="jspsych-video-slider-response-response"></div>';
    html += '</input><div>'
    ...
    function set_slider() {
      $('#jspsych-video-slider-response-response').slider();
      
      // hide all slider handles
      $('.ui-slider-handle').hide();
      
      $('#jspsych-video-slider-response-response').slider().on('slidestart', function( event, ui ) {
         // show the handle
         $(this).find('.ui-slider-handle').show();
         $('#jspsych-video-slider-response-next').prop('disabled', false);
      });
  }
